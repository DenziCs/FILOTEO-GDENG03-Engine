...
TexturedVertexBuffer::TexturedVertexBuffer() : AVertexBuffer::AVertexBuffer() {}
...
bool TexturedVertexBuffer::load(void* vertex_list, UINT vertex_size, UINT vertex_count, void* shader_byte_code, UINT shader_byte_size) {
	...
	HRESULT result = AGraphicsEngine::getInstance()->getD3DDevice()->CreateBuffer(&bufferDesc, &initData, &mBuffer);
	...
	result = AGraphicsEngine::getInstance()->getD3DDevice()->CreateInputLayout(layout, layoutSize, shader_byte_code, shader_byte_size, &mLayout);
	if (FAILED(result)) return false;
